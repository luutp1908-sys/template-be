...
const envSchema = Joi.object({
  ...
  EXPORT_SERVICE_URL: Joi.string().uri({ scheme: ['http', 'https'] }).optional().allow(''),
  ...
  DATABASE_POOL_MIN: Joi.number().integer().min(1).default(1),
  DATABASE_POOL_MAX: Joi.number().integer().min(1).default(10),
  DATABASE_POOL_TIMEOUT_MS: Joi.number().integer().min(1000).default(20000),
  ...
  REDIS_RETRY_DELAY_MS: Joi.number().integer().min(50).default(200),
  REDIS_KEEPALIVE_ENABLED: Joi.boolean().truthy('true').falsy('false').default(true),
  ...
  CACHE_AUTH_USER_TTL_MS: Joi.number().integer().min(1000).default(60000),
